import { Form, ColumnString, Event, GridColumn, Condition, RequestModel, Filter, SortCondition, Menu, Header } from '../../shared/interface/form-data-advanced';
...
@Component({
  selector: 'app-dynamic-tab-index',
  templateUrl: './dynamic-tab-index.component.html',
  styleUrls: ['./dynamic-tab-index.component.css']
})
export class DynamicTabIndexComponent  extends BaseComponent implements AfterViewInit {
  ...
  populateDataRecursively(currentPage: number, pageSize: number, jsonData: Array<any>, headerValues?: Array<ColumnString>, DynamicTabIndexComponent?: DynamicTabIndexComponent) {

    let formData_temp = DynamicTabIndexComponent == null ? this.formData : DynamicTabIndexComponent.formData;
    let utilityService_temp = DynamicTabIndexComponent == null ? this.utilityService : DynamicTabIndexComponent.utilityService;
  ...
  /*----------------------------------------POPULATE GRID DATA -----------------------------------------------------*/
  populateGridDataPagewise(currentForm1: DynamicTabIndexComponent, urlToCall: string,isSearch?:boolean, headerValues?: Array<ColumnString>,
    DynamicTabIndexComponent?: DynamicTabIndexComponent,TabHeader?: Array<Header>) {

    //Evaluate the columns by checking 'toPublish'
    let formData_temp = DynamicTabIndexComponent == null ? this.formData : DynamicTabIndexComponent.formData;
    let utilityService_temp = DynamicTabIndexComponent == null ? this.utilityService : DynamicTabIndexComponent.utilityService;
    let currentForm = DynamicTabIndexComponent == null ? this : DynamicTabIndexComponent;
    ...
    // merge the headers (input) with headerValues_temp, so that for tabbed pages it will consider the specific headerrequest
    //make sure this is applied before the cache
    let update_headerValues_temp =TabHeader != null?headerValues_temp.map((item, i) => Object.assign({}, item,TabHeader[i])):headerValues_temp;
    //find the RequestModel header and then evaluate for search and sort
    let requestModel = update_headerValues_temp.filter(t => t.Key == 'RequestModel');
    ...
    if (requestModel != null && requestModel.length > 0) {
      ...
      let requestModel_temp = cacheForPage != null && cacheForPage.RequestModel != null ? 
          cacheForPage.RequestModel : JSON.parse(requestModel[0].Value.toString().replace(/'/g, "\""));
  
          // let requestModel_temp =  JSON.parse(requestModel[0].Value.toString().replace(/'/g, "\""));
  ...
  //       JSON.parse(requestModel[0].Value.toString().replace(/'/g, "\""));
      ...
      let requestModel = cacheForPage != null && cacheForPage.RequestModel != null ? cacheForPage.RequestModel : new RequestModel();
      ...
      update_headerValues_temp.push(columnString);
    ...
    $('#salesProspectsId tbody > tr').html('');
    let response = utilityService_temp.getDataFormService(urlToCall,update_headerValues_temp);
  ...
  searchRecords() {
    ...
    this.formData.GridColumns.forEach(column => {
      if (column.IsSortable) {
        ...
        condition.OperatorSymbol = 6;
        condition.FieldName = column.FQSortableField;
        condition.FieldValue = this.searchValue;
  ...
  ngAfterViewInit() {
    ...
    this.formData.Sections.forEach(
      ...
      this.formData.Tabs.forEach(
        tab => {
              /*----------------------------------  EVENT BINDING DYNAMICALLY  IN TAB CLICK-------------------------------*/
              tab.Events.forEach(
                event => {
                  let x = document.getElementById(tab.TabName) as HTMLElement;
                  switch (event.EventName) {
                    case "onclick": {
                      if (x != null) {
                        console.log("call");
                        x.addEventListener('click', function () {
                       //   if (tab.RouteEntry != null) {
                            /*---------------------------------------- create a new url for GET data  using localStorage --------------------------------------*/
                            let completeUrl = utilityService.getApiUrl(module1) + '/' + event.EndPoint.EndpointAddress + '/' + currentObject.pageInfo1.pagesize + '/' + currentObject.pageInfo1.currentPage;
                            //populate grid data page wise
                            currentObject.populateGridDataPagewise(currentObject, completeUrl,null,null,null,event.EndPoint.Headers);
